...
function safeNext(user){
  if(!requested || !requested.startsWith("/") || requested.startsWith("//")) return I.destination(user?.role);
...
registerForm.addEventListener("submit",async e=>{e.preventDefault();message.textContent="Creating account…";try{const user=await I.register({firstName:registerForm.firstName.value,lastName:registerForm.lastName.value,email:registerForm.email.value,password:registerForm.password.value},true);location.replace(I.destination(user.role));}catch(err){message.textContent=err.message;}});
})();
